...
\caption{\label{simulation}Development of correlation between cultural and spatial distance for real world data and simulation results. The plots in the left column of the plot matrix (\textbf{ACE}) contain simulation results for an equal group distance matrix $I$, the ones on the right (\textbf{BDF}) for a distance matrix based on the macro unit distances defined for this paper (Figure \ref{map_regions} B). The rows of the plot matrix (\textbf{AB}, \textbf{CD}, \textbf{EF}) represent different degrees of intergroup transmission $m_i$. Each plot shows the results of 50 simulation runs. The x-axis displays the time distiguished into 200 year timeblocks, the y-axis Spearman's rank correlation coefficient between the mean squared euclidian distance matrix for a 200 year timeblock and the static spatial distance matrix. This information is visualized for each timeblock with (from left to right) a dotplot coloured according to the $p$-value of the Mantel test, a scatter plot line and a boxplot. The scatter plot points of one simulation run are connected with a faint line from one timeblock to the next. To compare the runs with the real world observations, the results for the burial customs data (Table \ref{tab:distanceandcorrelationtable*} AB) is added here with with a stronger green and blue line. The strong horizontal red line marks the correlation coefficient of zero, where neither positive nor negative correlation can be assumed.}
\end{figure*}
