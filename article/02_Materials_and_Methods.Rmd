...
\caption{\label{sed_region_matrix}Plot matrix of the region-region squared Euclidean distance time series. In the top right half for the variable \textit{burial type} and in the bottom left for the variable \textit{burial construction}. On the x-axis the time is displayed, on the y-axis the squared Euclidean distance between the two relevant regions. The faint, grey line shows the actual data, the stronger, coloured line an approximation (LOESS with $span = 0.3$).}
\end{figure*}
