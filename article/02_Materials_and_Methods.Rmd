...
From these time series another set of time series with a measure of cultural distance for each region-region relationship can be derived (Figure \ref{sed_region_matrix}) -- though with a reasonable amount of caution due to the partially low number of data available to reconstruct the developments in several regions and time slots. Cultural distance is defined here as the squared Euclidean distance (SED) between the trait proportions of two different regions at a certain point in time [@neimanStylisticVariationEvolutionary1995]. SED is a sensible metric in this context, because it is relatively simple, works with only a few assumptions and emphasises differences through squaring [@nakoinzFingerprintingIronAge2014].
...
Figure \ref{correlationtimeseries} shows the development of the correlation coefficient values obtained via Mantel statistics (Table \ref{tab:dacta} & \ref{tab:dactb}) during the succession of 200 years time slots.
...
knitr::kable(
  "latex",
  caption = "Like Table \\ref{tab:dacta}. \\textbf{C} shows the Mantel test results for the two variables if compared directly, \\textbf{D} the results of the Partial Mantel test considering and correcting for spatial distance as a second independent variable.",
  booktabs = T,
  linesep = "\\addlinespace \\hline \\addlinespace",
  escape = F
) %>%
...
\caption{\label{correlationtimeseries}Development of correlation between different measures of cultural and spatial distance. The results for the correlation of cultural and spatial distance (Table \ref{tab:dacta}) are displayed with a solid (\textit{burial type}) and a dashed (\textit{burial construction}) line. The results for the correlation of the two cultural distances (Table \ref{tab:dactb}) are displayed with a strong dotted line \textit{without} the correction for spatial distance (only here: Pearson correlation coefficient), and with a weak dotted line for the result \textit{with} the aforementioned correction. The horizontal line marks the correlation coefficient of zero, where neither positive nor negative correlation can be assumed.}
\end{figure}
