...
To get a better understanding how the correlation coefficient should develop if there was a strong correlation between spatial and cultural distance as measured by *burial type* and *burial construction*, a simple simulation was implemented based on the model proposed by @neimanStylisticVariationEvolutionary1995. This simulation assumes a population separated into $g = 8$ groups with a group population of $N_g = 100$ individuals each. $k = 2$ mutually exclusive ideas are randomly but equally distributed in this population. With every timestep all individuals learn from another randomly chosen individual and the total number of time steps is $t = 1400$ -- one for each year in the observed Bronze Age context. The probability that the other agent the individual is learning from comes from another group equals 
...
The partner group for each learning instance is selected randomly following a group interaction probability defined in the matrix 
...
I =
  \begin{dcases*}
    \begin{matrix}
      ...
      ... & ... & ... & ...
    \end{matrix} \\
    \text{Figure \ref{simulation} ACEG, equal distance between groups} \\
    \begin{matrix}
      0 & |b-a| & |c-a| & ... \\
      |a-b| & 0 & |c-b| & ... \\
      |a-c| & |b-c| & 0 & ... \\
      ... & ... & ... & ...
    \end{matrix} \\
    \text{Figure \ref{simulation} BDFH, spatial distance}
  \end{dcases*}
$$. 

The distribution of the $k$ variants can be observed for every timestep and documented in a time series like the one for real-world data.
