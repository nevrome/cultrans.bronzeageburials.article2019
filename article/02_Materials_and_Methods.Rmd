...
Regarding the variables burial type and burial construction in the grave dataset, the conditions shown in Table \ref{tab:gprcrosstab} apply. 

```{r gprcrosstab, echo=FALSE, tidy=FALSE}
load("../analysis/data/output_data/gprcrosstab.RData")
colnames(gprcrosstab)[1:2] <- c("Region", "Burial type")
gprcrosstab %>% 
  knitr::kable(
    caption = "Burial rite distribution in the active grave dataset. The order of the regions follows a southeast-northwest direction.",
    format = "latex",
    booktabs = T,
    escape = F,
    align = "l"
...
From these time series another set of time series with a measure of cultural distance for each region-region relationship can be derived (Figure \ref{sed_region_matrix}) -- though with a reasonable amount of caution due to the partially low number of data available to reconstruct the developments in several regions and time slots. Cultural distance is defined here as the squared Euclidean distance (SED) between the trait proportions of two different regions at a certain point in time [@neimanStylisticVariationEvolutionary1995]. SED is a sensible metric here, because it is relatively simple, works with only a few assumptions and emphasises differences through squaring [@nakoinzFingerprintingIronAge2014].
...
To test for correlation between the distances based on the variables *burial type* and *burial construction*, the time frame of interest was divided into sections of 200 years. For each of the $1400/200 = 7$ section and each of the two variables, a matrix of the mean SED over time was calculated. The two matrices of each section were tested for correlation with the Mantel test, which uses permutations of potentially correlating matrices to determine how strongly random reordering can alter an underlying correlation coefficient [@mantelDetectionDiseaseClustering1967]. Since both distances compared here, *burial type* and *burial construction* distance, are cardinal scale variables, the Pearson correlation coefficient [@pearsonNotesRegressionInheritance1895] can be applied as a base measure for the Mantel test (Table \ref{tab:dacta} C). 

Defining a measure of spatial distance for the above established main units of observation, the macro-regions, is only possible in a highly simplified way. A continuous distance metric such as least-cost path distance cannot be employed on this scale, but it is possible to distinguish ordinal distance classes. For the eight regions, four different categories of spatial distance were defined (Figure \ref{map_regions} B). The test for correlation between spatial and cultural distance works similar to the one between the two cultural distances above, except that the Mantel test has to be applied with Spearman's rank correlation coefficient [@spearmanProofMeasurementAssociation1904] here, because of the different data levels of the measurements (Table \ref{tab:dacta} AB).

```{r dacta, echo=FALSE, tidy=FALSE}
load("../analysis/data/output_data/distance_and_correlation_table_A.RData")
...
knitr::kable(
  ...
  kableExtra::add_header_above(
    c(" ", "burial type & spatial distance" = 3, "burial construction & spatial distance" = 3)
  ) %>%
  kableExtra::add_header_above(
    c(" ", "A" = 3, "B" = 3),
    bold = T, line = F
...
load("../analysis/data/output_data/distance_and_correlation_table_B.RData")
...
knitr::kable(
  "latex",
  caption = "Like Table \\ref{tab:dacta}. \\textbf{C} shows the Mantel test results for the two variables if compared directly, \\textbf{D} the results of the Partial Mantel test considering spatial distance as a second independent variable.",
  booktabs = T,
  linesep = "\\addlinespace \\hline \\addlinespace",
  escape = F
) %>%
  kableExtra::add_header_above(
    c(" ", "variable correlation" = 2, "variable correlation with spatial correction" = 2)
  ) %>%
  kableExtra::add_header_above(
    c(" ", "C" = 2, "D" = 2),
    bold = T, line = F
...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{../analysis/figures/correlation_time_series.jpeg}
\caption{\label{simulation}Development of correlation between cultural and spatial distance. The results for the burial customs data (Table \ref{tab:dacta} AB) are displayed with a solid (\textit{burial type}) and dashed (\textit{burial construction}) line. The horizontal line marks the correlation coefficient of zero, where neither positive nor negative correlation can be assumed.}
\end{figure}
