...
\caption{\label{map_graves}All graves dating between 2200 and 800calBC as extracted from the Radon-B database. The red frame indicates the research area.}
\end{figure}
...
\caption{\label{map_regions}Circular, artificial regions (\textbf{A}) and spatial distance network with classified distance values (\textbf{B}).}
\end{figure}
...
\caption{\label{development_burial_type}Regional distribution time series of the variable \textit{burial type} with the absolute amount (\textbf{A}) and proportion (\textbf{B}) of grave types.}
\end{figure}
...
\caption{\label{sed_region_matrix}Plot matrix of the region-region squared euclidian distance time series. In the top right half for the variable \textit{burial type} and in the bottom left for the variable \textit{burial construction}.}
\end{figure*}
...
knitr::kable(
  "latex", 
  caption = "Mean distance, correlation and mantel test results for cultural and spatial distance.", 
  booktabs = T,
  linesep = "\\addlinespace \\hline \\addlinespace",
  escape = F
) %>%
...
For the actual simulation runs, the parameters were chosen as follows: **TODO** Figure \ref{simulation*}

\begin{figure*}
\centering
\includegraphics[width=\textwidth]{../analysis/figures/simulation.jpeg}
\caption{\label{simulation}Simulation results in comparison with observed data.}
\end{figure*}
