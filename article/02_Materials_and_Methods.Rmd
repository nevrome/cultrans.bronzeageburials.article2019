...
gprcrosstab %>% 
  dplyr::mutate_at(
    dplyr::vars(tidyselect::one_of("flat", "mound", "unknown")),
    list(
      ~kableExtra::cell_spec(., "latex", bold = . > 100, color = ifelse(. < 10, "red", "black"))
    ) 
  ) %>%
  knitr::kable(
  ...
  kableExtra::collapse_rows(1, row_group_label_position = "identity", valign = "top")
